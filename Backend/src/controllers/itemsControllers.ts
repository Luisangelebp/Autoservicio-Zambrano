import { Request, Response } from "express";
import { db } from "../database";
import { Item } from "../models/itemsModel";

// Obtener todos los ítems
export async function getItems(req: Request, res: Response) {
  try {
    const items: Item[] = await db.all("SELECT * FROM items");
    return res.json(items);
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Error en el servidor" });
  }
}

// Obtener ítem por ID
export async function getItemById(req: Request, res: Response) {
  const { id } = req.params;
  try {
    const item: Item = await db.get("SELECT * FROM items WHERE id = ?", [id]);

    if (!item) {
      return res.status(404).json({ message: "Ítem no encontrado" });
    }

    return res.json(item);
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Error en el servidor" });
  }
}

// Crear ítem con foto
export async function createItem(req: Request, res: Response) {
  const { nombre, descripcion, precio, cant }: Item = req.body;
  const foto = req.file ? req.file.filename : undefined;

  if (!nombre || precio === undefined || cant === undefined || foto === undefined) {
    return res.status(400).json({ message: "Faltan campos obligatorios" });
  }

  try {
    const result = await db.run(
      `INSERT INTO items (nombre, descripcion, precio, cant, foto)
       VALUES (?, ?, ?, ?, ?)`,
      [nombre, descripcion || "", precio, cant, foto]
    );

    const newItem: Item = {
      id: result.lastID,
      nombre,
      descripcion,
      precio,
      cant,
      foto,
    };

    return res.status(201).json({
      message: "Ítem creado exitosamente 🚀",
      item: newItem
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Error en el servidor" });
  }
}

// Actualizar ítem con foto opcional
export async function updateItem(req: Request, res: Response) {
  const { id } = req.params;
  const { nombre, descripcion, precio, cant }: Item = req.body;
  const foto = req.file ? req.file.filename : null;

  try {
    const result = await db.run(
      `UPDATE items
       SET nombre = ?, descripcion = ?, precio = ?, cant = ?, foto = ?
       WHERE id = ?`,
      [nombre, descripcion, precio, cant, foto, id]
    );

    if (result.changes === 0) {
      return res.status(404).json({ message: "Ítem no encontrado" });
    }

    return res.json({ message: "Ítem actualizado exitosamente 🚀" });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Error en el servidor" });
  }
}

// Eliminar ítem
export async function deleteItem(req: Request, res: Response) {
  const { id } = req.params;

  try {
    const result = await db.run("DELETE FROM items WHERE id = ?", [id]);

    if (result.changes === 0) {
      return res.status(404).json({ message: "Ítem no encontrado" });
    }

    return res.json({ message: "Ítem eliminado exitosamente 🚀" });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Error en el servidor" });
  }
}